...
// Parámetros del PID
#define KP 7.0          // Ganancia proporcional
...
bool initialAngleSet = false;  // Variable para saber si el ángulo inicial ya se tomó
...
// Variables PID
float targetAngle = 1;  // Ángulo objetivo en grados (tomado del ángulo inicial)
float error;
...
void setup() {
  ...
  // Estabilización inicial (esperar unos segundos para que el robot esté quieto)
  delay(2000);  // Espera de 2 segundos antes de tomar el ángulo inicial
  
  // Leer varias veces el ángulo antes de comenzar
  for (int i = 0; i < 50; i++) {
    readMPU6050();
    anglez += (gz / 131.0) * 0.02; // Tomar lecturas cada 20 ms
    delay(20);  // Tiempo entre lecturas
  }

  targetAngle = anglez;  // Fijar el ángulo inicial como el objetivo
  initialAngleSet = true;
  Serial.print("Ángulo inicial fijado después de estabilización: ");
...
void loop() {
  ...
  readMPU6050();
  // Cálculo del ángulo actual en el eje Z
...
void controlMotor(int speedLeft, int speedRight) {
  ...
  if (speedRight > 0) {
    ledcWrite(CHANNEL3, speedRight-40); // Motor B1 adelante //ir chequeando eso para que funcione mejor
