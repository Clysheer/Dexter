...
struct{
float target;          // Objetivo del ángulo (normalmente 0 para equilibrar)
...
long tiempo_prev, dt;                          // Tiempo previo
...
void setup(*VMPU) {
  ...
  for (int cal_int = 0; cal_int < 1000; cal_int++) {  // Realizar la calibración 1000 veces
    ...
    delay(TIME);              // Esperar 3 milisegundos para permitir la calibración
  }
...
.
  ...
  angle += gx * 0.0000611 * dt;  // Actualizar el ángulo de pitch
  // Control PID
  error = target - angle;          // Calcular el error 
  derivative = (error - last_error) / dt;  // Calcular la parte derivativa
  angle = (KP * error) + (KD * derivative);  // Calcular la salida PID
